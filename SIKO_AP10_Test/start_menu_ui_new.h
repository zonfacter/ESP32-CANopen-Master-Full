...
struct StartMenu_Callbacks {
    ...
    std::function<void()> onIdentifyFirstFound = nullptr;
...
private:
    ...
    void createToolsScreen() {
        ...
        // Identify first found
        makeButton(m_screenTools, 470, 260, 220, 44, 0x4A2C82, "IDENTIFY", StartMenuUI::onIdentifyClicked);

        lv_obj_t* hint = lv_label_create(m_screenTools);
        lv_label_set_text(hint,
            "Ping Scan: SDO read 0x1000 auf Node 1..32 (ohne Baudwechsel).\n"
            "Identify: liest 0x1018 (Vendor/Product/Rev/Serial) vom ersten gefundenen Node."
        );
        lv_obj_set_style_text_color(hint, lv_color_hex(0xFFBB33), 0);
        lv_obj_set_style_text_font(hint, &lv_font_montserrat_14, 0);
    ...
    static void onIdentifyClicked(lv_event_t*) {
        if (!s_inst) return;
        Serial.println("[UI] IDENTIFY clicked");
        if (s_inst->m_cbs.onIdentifyFirstFound) s_inst->m_cbs.onIdentifyFirstFound();
    }
